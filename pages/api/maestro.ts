...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let resultStatements = await getLRSDataForNode(studentId, nodeId, true)
  ...
  if (node.title === "Teste de Nivelamento"){ //isPlacementTest
    const nextNodeIDGroup = {} as any;
    let shouldSetNextNodeId = false;
    if (!shouldWrite) {
      res.status(200).json({ placementTestDone: false })
      return;
    }
    let nextNodeId = "";
    words.forEach(wordData => {
      ...
      nextNodeId = wordData.destinationNodeID;
      const performance = lastOccurrenceOfWord?.ma5 ?? 0
      const occurrence = lastOccurrenceOfWord?.occurrence ?? 0

      if (performance < 1 && occurrence > 0){ // user did one mistake in the word
        if (nextNodeIDGroup[destinationNodeID]) { // its true when is a word in the same Group (destinationNodeID)
          return
        }else{
          nextNodeIDGroup[destinationNodeID] = true;
        }
      }
    })
    ...
    res.status(200).json({ shouldWrite, nodeComplete, nextNodeId })
    return
  }
  else {
